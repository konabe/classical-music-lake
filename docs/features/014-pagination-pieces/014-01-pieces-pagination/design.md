...
| #    | 箇所                                      | 指摘                                                                               |
| ---- | ----------------------------------------- | ---------------------------------------------------------------------------------- |
| R2-1 | `listPiecesQuerySchema` の `limit` 実装   | `union + transform + pipe` でなく Number 型ベースにまとめた方がシンプル            |
| R2-2 | `pages/pieces/index.vue` の状態表示と観測 | 状態 UI・センチネル・IntersectionObserver が template ではなく page に漏れ出ている |

### 対応方針（Round 2）

| #    | 対応                                                                                                                                                                                                                                                                                         |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R2-1 | `z.coerce.number().int().min().max().default()` に簡略化。API Gateway 由来の文字列入力は `coerce` で数値化し、`undefined` は `.default()` で既定値に解決される。テスト（417 件 / 境界値含む）で従来と同等の挙動であることを確認済み                                                          |
| R2-2 | Atomic Design のルール通り、新規 organism `PieceListInfinite.vue` を追加し、`PieceList` + センチネル + 状態 UI + `IntersectionObserver` を内包化。`PiecesTemplate.vue` はこの organism を利用する薄いテンプレートに、`pages/pieces/index.vue` は composable バインドと delete ハンドラのみに |
