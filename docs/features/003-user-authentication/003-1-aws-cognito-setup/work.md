...
3. **Lambda 関数が Cognito へアクセス可能（最小権限）**
   - Lambda 実行ロールに以下のアクションのみが許可されていること（最小権限の原則に従う）:
     - `cognito-idp:AdminGetUser`
     - `cognito-idp:AdminUpdateUserAttributes`
     - `cognito-idp:AdminCreateUser`
     - `cognito-idp:AdminDeleteUser`
     - `cognito-idp:ListUsers`
   - IAM ロール確認で各アクションが明示的に列挙されていること（`cognito-idp:*` は使用しない）
   - ポリシーの Resource が特定の User Pool ARN に限定されていること（ワイルドカード不可）
