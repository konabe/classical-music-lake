...
- **視聴ログの検索・統計**: 視聴ログ一覧のクライアントサイド絞り込みと、件数・評価分布・作曲家ランキング・月別トレンドの集計表示
...
- **現在**: 視聴ログ機能・コンサート記録機能（作成・一覧・詳細・編集・削除）、視聴ログのクライアントサイド検索フィルタと統計ダッシュボード
...
interface Piece {
  id: string; // UUID (自動生成)
  title: string; // 曲名
  composerId: string; // 作曲家マスタ（Composer）の id 参照（UUID）
  videoUrls?: string[]; // 動画 URL の配列（任意、最大 10 件。1 楽曲に複数演奏を登録できる）
...
>
> `videoUrl`（単一の動画 URL）は 2026-05 に `videoUrls`（動画 URL の配列）へ置換された。旧データは `DynamoDBPieceRepository` の読み込み時に透過的に `videoUrls: [videoUrl]` へ正規化されるため、明示的な移行 Lambda は持たない。書き込みは常に新スキーマで行うため、更新の発生したレコードから順に旧フィールドが消える。

#### バリデーション

- `title`: 空文字・空白のみ不可、最大200文字
- `composerId`: UUID 形式。Composer マスタ（`classical-music-composers`）の `id` を参照する。存在検証はサーバーでは行わず、UUID 形式のみ検証する（admin 限定 API かつ UI はセレクトのみのため）
- `videoUrls`: 任意項目。指定する場合は有効な URL 形式の配列であること（ドメイン制限なし、最大 10 件）。更新時に空配列 `[]` を送信するとフィールドが削除される
...
| クエリパラメータ | 型     | 必須 | 既定 | 説明                                                                         |
| ---------------- | ------ | ---- | ---- | ---------------------------------------------------------------------------- |
| `limit`          | number | -    | 50   | 1 ページあたりの件数。最小 1、最大 1000。範囲外は `400 Bad Request`          |
| `cursor`         | string | -    | なし | 前回レスポンスで返却された `nextCursor` を指定して続きを取得。不正値は `400` |
...
| `Url`          | `backend/src/domain/value-objects/url.ts`           | URL 形式のフィールド（`PieceEntity.videoUrls` の各要素 / `ComposerEntity.imageUrl` など）を表す。WHATWG URL パーサーで形式検証を行う |
...
任意項目として扱う URL 形式のフィールド（現状は `PieceEntity.videoUrls` の各要素 / `ComposerEntity.imageUrl`）の不変条件をドメイン層で保証する。

- 生成は `Url.of(value)`。内部で `value.trim()` を適用した上で、空文字は `RangeError`、文字列以外は `TypeError` を投げる
- 形式検証は WHATWG URL パーサー（`new URL(value)`）に委譲し、失敗した場合は `RangeError` を投げる。スキームの制限は行わない（Zod スキーマ `z.url()` と同等）
- `PieceEntity` は `videoUrls` 配列の各要素を `Url.of(...)` で VO 化し、内部 props で `Url[]` として保持する。`toPlain()` では `.map((u) => u.value)` で `string[]` に戻し、未設定時は `undefined` を返す
- `ComposerEntity` は `imageUrl` を `Url.of(...)` で VO 化する。`toPlain()` では `.value` で string に戻し、未設定時は `undefined` を返す
- DTO 境界（`types/index.ts` の `Piece.videoUrls` / `Composer.imageUrl`）は従来どおり primitive（それぞれ `string[] | undefined` / `string | undefined`）
...
- **検索機能（サーバー側）なし**: DynamoDB Scan のみ。視聴ログのみ `useListeningLogFilter` によるクライアントサイド絞り込みを提供（取得済みデータに対するキーワード／評価／お気に入り／期間フィルタ）。サーバー側の全文検索（OpenSearch 等）は将来フェーズ
- **統計（サーバー側）なし**: 視聴ログの統計ダッシュボード（`/listening-logs/statistics`）は `useListeningLogStatistics` によるクライアント集計で実現。データ件数が増えた場合はサーバー側集計や OpenSearch 連携を将来検討
...
- 多要素認証（MFA）
- 全文検索（OpenSearch 等によるサーバー側検索・現状はクライアントサイド絞り込みのみ）
- ページネーション
- アルバムカバー画像の管理
- タグ・カテゴリ機能
- データのエクスポート機能
- 統計・分析の高度化（サーバー側集計・年次比較・お気に入り作曲家の傾向分析等。基本ダッシュボードはクライアント集計で実装済み）
