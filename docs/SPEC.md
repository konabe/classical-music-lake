...
| `YouTubeThumbnail`         | 動画 URL を受け取り、YouTube URL の場合のみ `img.youtube.com/vi/{videoId}/mqdefault.jpg` のサムネイル画像を表示する Atom コンポーネント。画像ロードエラー時は自動的に要素ごと非表示になる                                                                              |
| `PieceCategoryList`        | 楽曲の4軸カテゴリ（ジャンル・時代・編成・地域）のうち設定済みのものを `CategoryBadge` で並列表示する Molecule コンポーネント                                                                                                                                           |
| `PieceItem`                | 楽曲一覧の各行コンポーネント。曲名・作曲家・`PieceCategoryList` でカテゴリバッジを表示する。`videoUrl` が YouTube URL の場合は左側（スマホでは最上段）に `YouTubeThumbnail` を表示し、クリックで詳細ページへ遷移する `detail` イベントを emit する                     |
