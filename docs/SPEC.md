...
| `Url`          | `backend/src/domain/value-objects/url.ts`           | URL 形式のフィールド（`PieceEntity.videoUrl` / `ComposerEntity.imageUrl` など）を表す。WHATWG URL パーサーで形式検証を行う |
...
任意項目として扱う URL 形式のフィールド（現状は `PieceEntity.videoUrl` / `ComposerEntity.imageUrl`）の不変条件をドメイン層で保証する。
...
- `PieceEntity` / `ComposerEntity` は `create()` / `reconstruct()` で入力値が存在する場合のみ `Url.of(...)` を呼び出し、VO を組み立てる。`toPlain()` では `.value` で string に戻し、未設定時は `undefined` を返す
- DTO 境界（`types/index.ts` の `Piece.videoUrl` / `Composer.imageUrl`）は従来どおり `string | undefined`
