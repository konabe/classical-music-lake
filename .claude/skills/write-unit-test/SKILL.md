...
| `describeInvalidBodyCases(handler, eventFactory)` | 共通のボディ異常系テスト（null / "null" / "[]" / "invalid json"）。`eventFactory: (body) => APIGatewayProxyEvent` |
| `makeLogRecord` / `makeLog` / `makePiece` / `makeComposer` | DTO・エンティティのファクトリー |

**新しい共通パターンに気づいたら fixtures に追加してから使う**。各ファイルに同じヘルパーを定義しない。

### 2.1 リクエストボディ異常系は `describeInvalidBodyCases` を必須で使う

**POST / PUT（リクエストボディを `parseRequestBody` でパースする）全ハンドラのテストには `describeInvalidBodyCases` を必ず入れる**。`null` / `"null"` / `"[]"` / `"invalid json"` の 4 ケースを各ファイルでインライン実装してはいけない（過去にこの 4 ケースを手書きした `describe("リクエストボディ異常系")` ブロックが多数のハンドラに重複していた）。

第 2 引数はイベントファクトリ `(body: string | null) => APIGatewayProxyEvent`。認証要件・HTTP メソッド・パスパラメータをファクトリ内で組み立てることで、認証必須ハンドラや PUT ハンドラにも使える。
...
// PUT（ローカル makeEvent ヘルパーがある場合はそれを再利用）
describeInvalidBodyCases(handler, (body) => makeEvent("abc-123", body));
```

新規ハンドラのテストを書くとき、または既存テストに `describe("リクエストボディ異常系")` のインライン実装を見つけたら、必ずこのヘルパー呼び出しに置き換える。
...
describe("POST /auth/login", () => {
  ...
  // 共通の異常系（POST/PUT ハンドラは必須。§2.1 参照）
  describeInvalidBodyCases(handler, (body) =>
...
1. **共通フィクスチャに移せるものを探す** — 複数ファイルで同じ helper が定義されていたら fixtures に移動。インラインの `describe("リクエストボディ異常系")` は `describeInvalidBodyCases`（§2.1）に置き換える
...
## 10. フロントエンド（Vue / Composable）の補足

§2〜§9 の多くはバックエンド前提（`fixtures.ts` は backend 専用）。フロントエンドのテストは以下を追加で守る。

- **テスト環境**: Vitest + `@nuxt/test-utils`
- **Composable**
  - API 呼び出しは `vi.fn()` でモックする
  - `mockNuxtImport("useRuntimeConfig", ...)` は使わない（Nuxt ルーター初期化を壊す）。テスト環境の実値を使う
  - 状態変化・エラーハンドリングをテストする
- **Vue コンポーネント**
  - `@nuxt/test-utils` の `mountSuspended` でマウントする
  - props / emit / slot をテストする
